...
The data includes the Reviews, Aggregate Ratings, Subject, Classes, and many more of all the professors on http://polyratings.com.
...
polyrating <- read_csv("https://raw.githubusercontent.com/ayakkala1/stat_final/master/vignettes/polyrating.csv") %>% 
                mutate(date = parse_date_time(date,"%m%y")) %>%
                drop_na()
```

The bread and butter of Text Analysis is tokenizing a corpus. This is to get a matrix of such that each row is a unique token of the corpus. A token can be a single word, combination of words, or even a sentence. We will be focusing right now on a single word or unigram for now.
...
For a lot of the following analysis we will take advantage of customizing the stopwords lexicon. Professor names appear a lot, so we remove that. We also were curious about bigrams that involve gender so we remove certain gender words from the stopwords to include them in our analyis.
...
To remedy this we will read in the stop words and perform an anti-join on our tokens. 
...
Let's end by comparing the Term Frequencies between Chemistry and Statistics to see which words are used a lot more in the Chemistry Reviews compared to the Statistics reviews. To start let's set up a dataframe to make term frequencies for each subject
...
AFINN scores unigrams between $(-5,5)$. As it seems, negative means a negative sentiment, positive means a positive sentiment, and zero is neutral. This is useful if you want work with sentiment as a quantitative variable. Such as using it in a scatterplot or regressor.
  ...
  inner_join(get_sentiments("bing")) %>%
    ...
    count(date, subject, sentiment) %>%
      ...
      ggplot(aes(date,sentiment,fill=subject)) + geom_col(show.legend = FALSE) +
             ...
             theme(panel.spacing.x=unit(1, "lines"),plot.margin = margin(.3, .8, .3, .8, "cm")) +
             xlab(element_blank()) + ylab("Sentiment")
```

Furthermore using Bing again let's see what words contributed most to the Postive Sentiment and to the Negative Sentiment. Let's first get our token counts with sentiment joined.
...
As you may have guessed, counts may not give the most useful information. Before, we say this when we just say the most frequent words in the corpus. Words like "the" and "a" were given, when in reality they just are simple conjuctions. Instead of removing them through anti joining stop words, we can use a totally new metric.

Let's even look at how the Term Frequencies are distributed between COSAM Subjects.
...
..and then see the top tf-idf n-grams for each COSAM section.
